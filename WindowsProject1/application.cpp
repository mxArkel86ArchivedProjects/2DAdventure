...
X::Rect schemToLocalZ(X::Rect r, X::Rect PLAYER_SCREEN_LOC, X::Point location, double gridsize, double z_inp, int width, int height)
{
	double z = exp(z_inp);
	X::Point p = X::Point(
...
{
	ID2D1Bitmap *bitmap;
	LoadBitmapFromFile(pRT, pWICFactory, PCWSTR(path.c_str()), size, size, &bitmap);
...
void Application::Paint()
{
	...
	{
		if (resources[p->res()] != NULL && p->getZ() < 0.000)
		{
	...
	if (showprops)
	{
		for (LevelProp *p : props)
		{
			if (resources[p->res()] != NULL && p->getZ() > 0.000)
			{
				X::Rect obj = schemToLocalZ(*p, PLAYER_SCREEN_LOC, location, gridsize, p->getZ()== 0.001?0.000:p->getZ(), (int)SCREEN_BOUNDS.right(), (int)SCREEN_BOUNDS.bottom());
				pRT->DrawRectangle(D2D1::RectF(obj.left() - 4, obj.top() - 4, obj.left() + 4, obj.top() + 4), colors["red"], 2.0);
				if (!CollisionUtil::staticCollision(obj, SCREEN_BOUNDS))
					continue;
